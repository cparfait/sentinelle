...
def test_contrat_porte_imputation_et_bdc(client):
    """Deux colonnes du tableau Excel des marchés : l'imputation budgétaire et
    le bon de commande signé, qui existaient déjà sur un certificat. Le
    service demandeur, le responsable et la priorité, eux, ont quitté le
    contrat le 26/09/2026 : rien n'en dépendait."""
    r = client.post('/contracts/create', data={
        'name': 'Cotisation ADULLACT', 'notice_days': '0',
        'budget_code': '65818', 'order_signed_on': '2026-03-13'}, follow_redirects=True)
    assert r.status_code == 200
    ...
    assert '65818' in body and '13/03/2026' in body
    # plus de champ service, responsable ni priorité au formulaire
    form = client.get(f'/contracts/{ct.id}/edit').data.decode('utf-8')
    assert 'name="service_id"' not in form and 'name="responsible"' not in form and 'name="priority"' not in form


def test_les_deux_champs_se_vident(client):
    ...
    client.post(f'/contracts/{ct.id}/edit', data={
        'name': 'Fibre', 'notice_days': '0',
        'budget_code': '', 'order_signed_on': ''},
        follow_redirects=True)
    assert ct.budget_code is None and ct.order_signed_on is None
